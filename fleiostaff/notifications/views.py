...
from fleio.notifications.serializers import DispatcherLogDetailSerializer, DispatcherLogSerializer
from fleio.notifications.models import DispatcherLog, NotificationTemplate, UserNotificationsSettings
...
class StaffNotificationViewset(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        return (DispatcherLog.objects
                .filter(notification__user=self.request.user).order_by('generated'))
    ...
    def get_user_notifications_settings(self, request):
        ...
        response_data = {}
        templates = NotificationTemplate.objects.exclude(name__contains='enduser').order_by('name').all()
