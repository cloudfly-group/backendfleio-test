...
class StaffPortSerializer(serializers.ModelSerializer):
...
class StaffPortCreateSerializer(serializers.ModelSerializer):
    ...
    def to_internal_value(self, data):
        ...
        """
        value = super(StaffPortCreateSerializer, self).to_internal_value(data)
        if 'mac_address' in value and not value['mac_address']:
            value.pop('mac_address')
        return value


class StaffPortUpdateSerializer(serializers.ModelSerializer):
    id = serializers.CharField(read_only=True)
    ...
    def to_internal_value(self, data):
        ...
        to_internal_value is called before .validate() and after each field is validated.
        """
        value = super(StaffPortUpdateSerializer, self).to_internal_value(data)
